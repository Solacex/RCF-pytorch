...
def main():
    ...
    multiscale_test(model, test_loader, test_list=test_list,
            save_dir =args.tmp)



def multiscale_test(model, test_loader, test_list, save_dir):
    model.eval()
    if not isdir(save_dir):
        os.makedirs(save_dir)
    scale = [0.5, 1, 1.5]
    for idx, (image,name) in tqdm(enumerate(test_loader)):
        name = name[0]
        name = name.split('/')[-1]
#        image = image[0]
 #       image = image.squeeze()
...
#        multi_fuse = (multi_fuse - multi_fuse.min()) / (multi_fuse.max() - multi_fuse.min())
        ...
        result_out.save(join(save_dir, name))
